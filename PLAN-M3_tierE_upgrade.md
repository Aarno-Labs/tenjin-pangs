# M3 Tier-E Upgrade Plan — Demand-driven CFL queries & on-the-fly call-graph fixpoint

**Status:** archived upgrade-path plan. This is the full-design tier-E plan from
`DESIGN.md`, not the active PANGS-lite M3 plan. Under `DESIGN_lite.md`, tier E is cut;
M3 first measures the lite pipeline and only revives this plan if coverage is limited by
context-insensitive indirect-call residue that is not already Ω-frozen.
